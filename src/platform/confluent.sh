...
config_kafka() {
   ...
   config_service "kafka" "kafka" "server" "metric.reporters" "io.confluent.metrics.reporter.ConfluentMetricsReporter"

   get_service_port "listeners" "${confluent_conf}/kafka/server.properties"

   if [[ -n "${_retval}" ]]; then
      config_service "kafka" "kafka" "server" "confluent.metrics.reporter.bootstrap.servers" "${_retval}"
   else
      config_service "kafka" "kafka" "server" "confluent.metrics.reporter.bootstrap.servers" "9092"
   fi

   config_service "kafka" "kafka" "server" "confluent.metrics.reporter.topic.replicas" "1"
}
...
start_control-center() {
    ...
    is_running "kafka" "false" \
        || die "Cannot start Confluent Control Center, Kafka Server is not running. Check your deployment"
...
config_control-center() {
    ...
    config_service "control-center" "confluent-control-center" "control-center" \
    ...
    config_service "control-center" "confluent-control-center" "control-center" \
        "cconfluent.controlcenter.command.topic.replication" "1"
}

export_control-center() {
    get_service_port "confluent.controlcenter.rest.listeners" "${confluent_conf}/confluent-control-center/control-center.properties" "="
    if [[ -n "${_retval}" ]]; then
...
wait_control-center() {
    ...
    wait_process_up "${pid}" 4000 || echo "Confluent Control Center failed to start"
}
